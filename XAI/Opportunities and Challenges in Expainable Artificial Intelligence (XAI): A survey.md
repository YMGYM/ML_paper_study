...
- 하지만 인공 신경망의 블랙박스 특징 때문에 사용이 제한되는 분야가 발생한다.
- 설명가능 인공지능(XAI) 기술은 인공지능의 결정에 대해 해석적이고, 직관적이며, 이해가능한 설명을 제공한다.
- 본 논문은 XAI를 scope, methodology, explanation level에 따라 정리하며 설명했다.
- XAI 연구분야에서 2007년 부터 2020년까지 역사적 타임라인을 보여 주며, 연구를 정리했다.
- 8개의 XAI알고리즘의 explanation maps 를 평가하며, 한계점과 미래 발전 방향에 대해 논할 것이다.
...
- 심층 신경망의 결정을 해석하는 것은 관련 지식을 필요로 한다
- 이런 지식들은 AI 비전문가나, 최종사용자와 같이 정확한 결과가 더 중요한 사람들에게는 필요가 없는 지식이다.
...
1. XAI를 시간 순서대로 파악하고, 세가지 카테고리로 분류해 명확성과 접근성을 올린다.
2. XAI 연구의 핵심 수학적 모델을 요약하고 분류하며 한계와 발전 방향을 평가한다.
3. 8개의 XAI알고리즘의 explanation maps를 생성하고 분석해 본다.
...
- 연구는 Google Scholar, ACM Digital Library, IEEEXplore, ScienceDirect, Spinger, arXiv 등에서 참고하여 정리하였다.
...
본 논문은 XAI를 `Scope`, `Methodology`, `Useage` 의 세 가지 관점에서 분류했다.
...
2. Methodology 
   ...
   - 설명가능 모델 내부에 존재하는 핵심 알고리즘의 방향성을 말한다.
   - Backpropagation-based  : 역전파 과정중, 편미분을 활용하여 설명력을 얻는다.
   - Perturbation-based : occlusion, 부분적으로 제거된 피쳐, 마스킹, 조건부 샘플링 등의 방법을 사용한다. 역전파가 필요 없다는 장점이 있다.

3. Usage

   - 특정 모델에만 적용될 수 있는지, 외부 알고리즘에도 적용될 수 있는지를 말한다.
...
- Section 7에서, XAI알고리즘의 정량적, 정성적 평가를 위해 몇가지 평가 알고리즘을 제안했다.
...
- 많은 사용자가 기여한 알고리즘이 포함된 패키지들을 선택했다.
...
- 신경망을 설명하기 위한 접근이 많은 연구에서 제안된 바 있다.
- 신경망에 적용된 설명가능 인공지능의 목표는 다양한 방법으로 모델의 예측값을 설명하는 것에 집중한다.

![survey_3](https://github.com/YMGYM/ML_paper_study/blob/main/XAI/Images/survey_3.png?raw=true)

- 이것은 논문에서 DNNs를 설명하기 위해 설명한 그림이다.

- 딥러닝 모델은 하나의 $x$ 가 딥러닝 모델 $f$를 통과하여 $\bar{y}$ 라는 결과를 산출한다.

- 이때 $x$는 전체 데이터 안의 단일 데이터이며, 다른 메타데이터가 포함되어있지 않는다.
- $x$가 딥러닝 모델을 통과하여 $\bar y$를 반환하는데, 이는 특정 클래스 $C$에 포함된 결과이다.
- 이 신경망 모델을 $\bar y = f(\theta, x) $ 로 표현한다.
...
- 해석가능한 도메인은 이미지나 텍스트 등 사람이 이해할 수 있는 도메인들이 있다.
...
- 예를 들어 이미지에 대해서는 입릭 이미지와 같은 크기의 픽셀 맵인 explanation map이 생성될 수 있다.

- 텍스트에 대해서는 word-by-word influence scores가 있다.
...
- 웹 기반 딥러닝 모엘 서비스나, 제한된 비즈니스 플랫폼에서의 API식ㄴ 모델은 입력 데이터를 받아 단순히 결과물만 반환하는 블랙박스적 구성을 띄고 있다.



추가적으로 아래 용어는 XAI가 중요한 이유와 함께 설명하고 있다.

인공지능이 윤리성, 사법성, 안전성을 가지기 위해 필요한 것들이다.
...
첫 번째 예시로 적대적 공격을 이야기하며 다음 사진을 보여 주고 있다.  원본 이미지의 약간의 적대적 노이즈를 더해주니 판다를 긴팔원숭이로 이해하게 되는 문제를 보였다.

![survey_5](https://github.com/YMGYM/ML_paper_study/blob/main/XAI/Images/survey_5.png?raw=true)

두 번째 예시로 이미지를 분류할 때 워터마크에 집중하여 분류하는 모습을 보일 수 있다. 텍스트를 제거하니 정상적인 분류가 아닌 값을 반환한다.

인공지능 모델이 텍스트에 대해 학습된 것이다.



논문은 우리가 자동화된 알고리즘에 의존할 수록 AI알고리즘이 공격을 방어하고 투명한 결과를 제공하는 것은 매우 중요해져야 할 것이라고 말한다.
...
아무 설명이 없는 최적의 결정보다 덜 합리적인 결정에 대해 과학적 설명/논증이 있는 것이 더 낫다.

따라서 '특정 결정이 왜 내려졌는가' 는 최종 사용자가 신뢰감을 가지는 데 매우 중요한 요소가 된다.

XAI를 통한 Fundamental explanations 는 이해관계자나 정부 대리인들에게 신뢰를 주고, socio-economic 환경에서 AI를 연결할 수 있게 한다.
...
이후 본 논문은 다양한 기법을 제시해 보며 논의해볼 것이다.
...
- 의사는 인공지능의 예측에 대해 깊은 예측과 '왜 그렇게 생각했는지' 명확한  설명이 필요하다.
- 위 사례와 같은 유형의 문제가 Locally explainable한 방법이다.



로컬 설명은 모형 $f$와 단일 데이터 $x$를 통해 설명 내용인 $g$를 만드는 것이다.
...
- 이 결과들은 항상 양수인 real-valued metrics/vectors였다는 문제점이 있었다.
- 최근 연구는 attribution maps나 그래프 기반, 게임이론 기반 등을 사용해 양수와 음수의 결과를 모두 사용한다.
- 주로 양의 분포값은 특정 피처가 결과가 나오는데 긍정적 영향을 미친 것이고, 음의 분포값은 결과의 부정적인 영향을 미치는 것이다.
...
- 특정 채널의 activation을 최대화 하는 패턴x가 있다면, 그 x가 채널이 학습한 피처의 패턴이라고 볼 수 있다는 아이디어이다.
- 실행 과정은 다음과 같다.
...
- 샐리언시 맵은 사람이 물체를 볼 때 관심 영역에 시선을 먼저 집중하는 데서 아이디어를 얻었다.
- 샐리언시 맵은 출력 카테고리에 대한 모델의 그래디언트를 계산한다.
- 그래디언트를 시각화하면 픽셀에 대한 중요도를 볼 수 있을 것이다.
- 이 때 **양의 그래디언트를 가지면 출력 결과에 더 큰 영향을 줄 것이라는 가정을 수행**한다.
- 저자는 1) 클래스 모델 시각화, 2) image-specific 시각화의 두 가지 시각화 방법을 제안했다.
- 1) 클래스 모델 시각화는 글로벌 기법이므로 후에 설명한다.
...
- 하나의 뉴런의 출력은 그 노드에 대한 타당성 점수의 합으로 나타내어진다. $ f(x) = \Sigma_jR_{i\leftarrow j}$
...
- 한 계층의 타당성 점수의 합은  이전 계층의 모든 타당성 점수의 합으로 나타내어진다. (보존 공식)
- 이렇게 얻은 마지막 계층의 타당성 점수를 시각화하면 중요도를 시각적으로 표현할 수 였을 것이다.
- LRP 방법은 모델의 정확도 손실 없이 연산량이나 저장 비용 등을 절감하는 network pruning으로도 사용될 수 있다.
...
- LIME은 '중요한 부분일 수록 변형되었을 때 예측값이 크게 변한다'는 아이디어이다.
- 입력을 변형시키면서 예측값을 크게 변화시키는 값을 찾는 방법이다.

- LIME은 연속적인 '슈퍼픽셀'을 통해 중요성을 찾으려 하는 방법이다.
- 0 (목표값이 없다) 또는 1(목표값이 있다) 을 예측하는 방법 대리 모델을 생성하는 방법으로 문제를 단순화 한다.
- 위 데이터는 이미지에 대해서는 다음과 같이 나타낼 수 있다. 
...
- LIME는 위 식 (  $\xi(x)$  )을 최소화하는 방향으로 학습을 진행한다.
- $\mathcal{L}(f,g,\pi_x)$ 는 $f(x)$ 와 $g(x)$ 의 차이이다. (예측 값이 비슷할수록 중요한 데이터가 포함되어 예측 오차가 작아진다.)
...
- [**Quadratic-LIME(QLIME)** ](https://scholar.smu.edu/cgi/viewcontent.cgi?article=1138&context=datasciencereview): 대리 모델을 비선형으로 설정해도 LIME을 적용할 수 있게 했다.
...
- SHAP는 input $x$가 예측값에 미친 피처별 기여도를 설명한다.
- 섀플리값은 '연합 게임이론'에서 플레이어의 기여도에 따라 보상을 배분하는 방법에 대한 표현이다.
- 기여도를 분석하는 피처는 테이블 형태에서는 각각의 행이, 이미지에서는 슈퍼픽셀이 될 수 있다.
- $g(z′)=ϕ_0+∑_{j=1}^{M}ϕ_jz′_j$ 의 식으로 설명되는 방법이다.
...
- 글로벌하게 설명하는 방법은 딥러닝 모델을 더 해석하기 쉬운 선형 모델로 추론하는 방법이다.
- Rule-based나 트리 모형, 선형 모델은 그 자체로도 글로벌하게 해석가능한 모델이다.
- 일반적으로 입력들의 대해 블랙박스 모형의 전반적인 경향을 보여주게 된다.
...
- TCAV는 입력 특징이 적절하게 선택되지 못하면 좋은 결과를 얻지 못한다는 단점이 있다.
- 서로 관련성이 높은 특징 또한 성능을 감소시킬 수 있다.
...
- SpRAy는 LAP설명법을 기반으로 한 로컬 인스턴스의 최상단 부분을 설명하는 기법이다.

- LRP 모델에서 자주 나오는 특징을 분석하게 된다.

- 자세한 설명은 알고리즘을 참고해서 보면 도움이 될 것 같다.

  <img src="https://github.com/YMGYM/ML_paper_study/blob/main/XAI/Images/survey_14.png?raw=true" alt="survey_14" style="zoom:67%;" />

- LRP를 통과한 결과값에 대해 Specral Cluster Analysis(SC) 와 EigenMapAnalysis를 수행하는 것을 확인할 수 있다.
...
- 한 피처에 주목하는 하나의 인공 신경망을 각각 만들어서 합치는 방법으로 모델을 설명한다.
